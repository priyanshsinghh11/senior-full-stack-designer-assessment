  ...
  async function onSubmit(values: CandidateForm) {
    ...
    try {
      ...
      if (!response.ok || !candidate.id) {
        throw new Error(candidate.error || "Candidate information could not be saved.");
      }
        ...
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-5">
          ...
          <section className="card">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 p-6">
              <div>
                <div className="mb-4 flex items-center gap-2 text-[15px] font-semibold text-[var(--ink-900)]">
                  <UserRound className="h-4 w-4 text-[var(--sky-500)]" />
                  Candidate details
                </div>
                ...
                <div className="grid gap-4">
                  ...
                  <Field label="Email address" error={errors.email?.message} required>
                    <div className="relative">
                      <Mail className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--sky-500)]" />
                      <input
                        {...register("email")}
                        suppressHydrationWarning
                  ...
                  >
                    <div className="relative">
                      <LinkIcon className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--sky-500)]" />
                      <input
                        {...register("resumeLink")}
                        suppressHydrationWarning
                        className="field-input pl-10"
              ...
              <div className="flex flex-col gap-3 border-t border-[var(--line-light)] pt-6 sm:flex-row sm:items-center sm:justify-between">
...
function StatusMessage({
  ...
  return (
    <div
      className={`flex items-start gap-2 px-4 py-3 text-[14px] font-medium ${isError ? "text-[var(--danger)]" : "text-[var(--ink-900)]"
        }`}
      style={{
        borderRadius: "var(--r-2)",
        background: isError ? "rgba(192, 52, 29, 0.08)" : "var(--sky-50)",
        border: "1px solid var(--line-light)",
      }}
    >
      {isError ? null : <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[var(--ink-600)]" />}
      <span>{message}</span>
    </div>
  );
}
